...
<div bind:this={cellRef} class="w-full h-full min-h-0 flex items-center justify-center">
	...
	>
		...
		{#if effectiveTitle}
			<div
				class="absolute bottom-0 left-0 z-[2] text-sm md:text-xl font-medium text-left pl-4 pr-4 md:pr-6 pt-2.5 pb-[13px] pointer-events-none {effectiveTitleColor === 'black' ? 'text-black [text-shadow:0_0_3px_rgba(255,255,255,0.45)]' : 'text-white [text-shadow:0_0_3px_rgba(0,0,0,0.45)]'}"
			>
				{effectiveTitle}
			</div>
		{/if}
